  ...
  function c(e) {
    ...
    function u(e) {
      for (_ = 0; _ < J.length; _++)
        (G = J[_]),
          o(G)
            ? O[(e ? "add" : "remove") + "EventListener"].call(O, G, k, !1)
            : t(G)
            ? (k = G)
            : (O = G);
    }
      ...
      C = {
        ...
        create: function (e) {
          return (
            ...
            (l = e.element =
              e.element || y.createElement(e.type === m ? "div" : "canvas")),
            (s = e.context =
              e.context ||
              (function () {
                switch (e.type) {
...
if (document.getElementById("clickCanvas")) {
  ...
  clickparticle.spawn = function (x, y) {
    ...
    particle.init(x, y, random(5, 15)); 
    particle.wander = random(0.5, 2.0);
    particle.color = random(COLOURS);
    particle.drag = random(1.05, 0.99);
    theta = random(TWO_PI);
    force = random(2, 5);
  ...
  document.addEventListener("mousedown", function (e) {
    ...
    "TEXTAREA" !== e.target.nodeName &&
      ...
      "I" !== e.target.nodeName &&
      (function () {
        for (max = random(10, 100), j = 0; j < max; j++)
